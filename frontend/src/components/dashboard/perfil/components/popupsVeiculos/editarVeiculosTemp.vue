<template>
    <div class="popup-container" @click="closePopupOnOverlay" v-if="showEditarVeiculoPopup">
        <div class="popup-content">
            ...
            <div class="form-container">
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" id="modelo" v-model="modelo" maxlength="100" required>
                </div>
                <div class="form-group">
                    <label for="placa">Placa</label>
                    <input type="text" class="form-control" id="placa" maxlength="10" v-model="placa" required>
                </div>
                <div class="form-group">
            ...
            <div class="form-group" style="margin-top: 5%;">
                ...
                <div class="draggable-area" :class="{ 'dragging': isDragging }" @drop.prevent="handleDrop"
                    ...
                    <p v-if="!foto && !newFoto">Arraste e solte a imagem aqui ou clique para selecionar um arquivo</p>
                    <img :src="foto" v-if="foto && foto != 'semFoto' && !newFoto" alt="Imagem do Veículo" class="uploaded-image">
                    <img v-else :src="imageUrl" v-if="newFoto" alt="Imagem do Veículo" class="uploaded-image">
                </div>
            </div>
            <div class="buttons">
                <button @click="closePopup">Cancelar</button>
                <button @click="editarVeiculo">Salvar Alterações</button>
            </div>
        </div>
    </div>
...
export default {
    emits: ['close'],
    props: {
        showEditarVeiculoPopup: Boolean,
        data: Object
    },
    components: {
        Mensagem,
    ...
    computed: {
        imageUrl() {
            return this.newFoto ? URL.createObjectURL(this.newFoto) : '';
        }
    },
    methods: {
        fecharModal() {
        ...
        editarVeiculo() {
            ...
            axios.put(`${store.state.apiUrl}/veiculos/${this.data.idVeiculo}`, formData, {
    ...
    mounted() {
        if (this.data) {
            ...
            if (this.data.foto && this.data.foto != 'semFoto') {
                this.foto = `${this.apiUrl}/public/files/${this.data.foto}`
