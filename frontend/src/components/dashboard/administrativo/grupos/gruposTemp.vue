<template>
    ...
    <Mensagem :mensagem="mensagemAlerta" v-if="!mensagemErro && !mensagemSucesso" tipo="Alerta"
        @fechar-modal="fecharModal" />
    <cadastroGruposPopup :showcadastroGruposPopup="showcadastroGruposPopup"
        v-if="showcadastroGruposPopup" @close="closecadastroGruposPopup" />
    <editarGruposPopup :data="dataEdicao" :showEditarGruposPopup="showeditarGruposPopup"
        v-if="showeditarGruposPopup" @close="closeeditarGruposPopup" />
    <deletarGruposPopup :data="dataExclusao" :showDeleteGruposPopup="showDeleteGruposPopup"
        v-if="showDeleteGruposPopup" @close="closeDeleteGruposPopup" />
          ...
          <div class="table-container">
            <div class="headerTable">
              <div class="cadastro"><button type="button" class="botaoPrimario"
                  @click="openCadastroGruposPopup">Novo <i class="fa fa-plus"
            ...
            <table>
              <thead>
                <tr>
                  <th scope="col" class="acoes">ID</th>
              ...
              <tbody>
                <tr v-for="(item, index) in paginatedItems" :key="index">
                  ...
                  <td data-label="Fretes">{{ item.planos.tipo }}</td>
                  <td data-label="Ações">
                    ...
                    <button type="button" title="Deletar" class="btn" @click="openDeleteGruposPopup(item)"><img
...
import cadastroGruposPopup from './popups/cadastroGruposPopup.vue'
import deletarGruposPopup from './popups/deletarGruposPopup.vue'
import editarGruposPopup from './popups/editarGruposPopup.vue'
import { ref } from 'vue';
...
export default {
    ...
    data() {
        return {
            ...
            mensagemSucesso: '',
            mensagemAlerta: '',
            showDeleteGruposPopup: false,
            showeditarGruposPopup: false,
            showcadastroGruposPopup: false,
            semResultado: false,
            itemsPerPage: 10,
            currentPage: 1,
            totalItems: 0,
    ...
    methods: {
        ...
        openCadastroGruposPopup() {
            var html = document.documentElement;
            html.classList.add('no-scroll');
            this.showcadastroGruposPopup = true;
        },
        closecadastroGruposPopup() {
            var html = document.documentElement;
            html.classList.remove('no-scroll');
            this.getgrupos();
            this.showcadastroGruposPopup = false;
        },
        openeditarGruposPopup(data) {
            var html = document.documentElement;
            html.classList.add('no-scroll');
            this.dataEdicao = data;
            this.showeditarGruposPopup = true;
        },
        closeeditarGruposPopup() {
            var html = document.documentElement;
            html.classList.remove('no-scroll');
            this.getgrupos();
            this.showeditarGruposPopup = false;
        },
        openDeleteGruposPopup(data) {
            var html = document.documentElement;
            html.classList.add('no-scroll');
            this.dataExclusao = data;
            this.showDeleteGruposPopup = true;
        },
        closeDeleteGruposPopup() {
            var html = document.documentElement;
            html.classList.remove('no-scroll');
            this.getgrupos();
            this.showDeleteGruposPopup = false;
        },
        async getgrupos() {
            this.loading = true;
            this.grupos = []
            ...
            await axios.get(`${store.state.apiUrl}/public/grupos?&start=${start}&quantity=${this.itemsPerPage}`, {
