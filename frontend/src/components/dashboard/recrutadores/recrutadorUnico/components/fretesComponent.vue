<template>
    ...
    <div class="containerSmall">
        ...
        <div class="title"><span>Fretes</span></div>
...
export default {
    ...
    methods: {
        ...
        async getFretes() {
            ...
            try {
                const response = await axios.get(`${store.state.apiUrl}/public/fretes?&start=${start}&quantity=${this.itemsPerPage}&idRecrutador=${this.idRecrutador}`, {
                    withCredentials: true,
                });

                if (response.status === 200) {
                    let fretes = response.data.fretesRecrutadores
                    if (fretes.length == 0){
