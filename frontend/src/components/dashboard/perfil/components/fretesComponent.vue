<template>
    ...
    <div class="containerSmall">
        ...
        <cadastroFretesPopup :showCadastroFretesPopup="showCadastroFretesPopup" v-if="showCadastroFretesPopup"
            @close="closeCadastroFretesPopup" />
        <div class="title"><span>Anunciar Fretes</span><button @click="openCadastroFretesPopup"><span>Cadastrar</span>
            ...
            <div v-for="(frete, index) in fretes" :key="index" :class="{ 'card': true, 'loading': loading }">
                ...
                <div class="buttons">
                    <div style="text-align: right;">
                        ...
                        <select class="form-control" v-model="frete.status"
                            @change="() => atualizarStatus(frete.idFretes, frete.status)">
...
import cadastroFretesPopup from './popupsFretes/cadastroFretesTemp.vue'
import PopupFilters from './popupsFretes/filtrosPopup.vue';
...
export default {
    ...
    methods: {
        async atualizarStatus(id, status) {
            ...
            await axios.put(`${store.state.apiUrl}/fretes/updateStatus/${id}`, data, {
        ...
        async getFretes() {
            ...
            try {
                const response = await axios.get(`${store.state.apiUrl}/fretes?&start=${start}&quantity=${this.itemsPerPage}&de=${de}&para=${para}&distancia=${distancia}&valor=${valor}&tipo=${tipo}`, {
                    withCredentials: true,
                });
                ...
                if (response.status === 200) {
                    ...
                    this.fretes = await this.encontrarCidades(fretes);
